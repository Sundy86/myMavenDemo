...
public class TestDataByExcelFile  extends BeforeAndAfter {
    private static  String path=System.getProperty("user.dir")+File.separator+"testdata"+File.separator;
    @DataProvider(name="searchData")
    public static Object[][] data() throws IOException
    {
        //获取Excel文件的测试数据
        return getSearchData(path, "testData.xlsx","testData");
    }
    ...
    public void testSearch(String searchdata1,String searchdata2,String searchResult) {
        ...
        //打开sogou首页
        chrome_driver.get("http://www.sogou.com/");
        //输入搜索条件
        //从Excel文件中读取每行中前2个单元格内容作为搜索框中输入的搜索关键词,在两个搜索词中间增加一个空格
        chrome_driver.findElement(By.id("query")).sendKeys(searchdata1+" "+searchdata2);
        //单击搜索按钮
        chrome_driver.findElement(By.id("stb")).click();
        try{
            Thread.sleep(3000);
...
//            }});
        ...
        Assert.assertTrue(chrome_driver.getPageSource().contains(searchResult));
    }
