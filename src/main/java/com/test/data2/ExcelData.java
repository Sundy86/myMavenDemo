...
public class ExcelData {
    ...
    //从Excel文件获取测试数据的静态方法
    public static Object[][] getData1(String filePath,String FileName,String sheetName) throws IOException{
        //  LogUtil.logInfo(filePath);
        //根据参数传入的数据文件路径和文件名称，组合出Excel数据文件的绝对路径，声明一个File文件对象
        File file = new File(filePath + FileName);
        //创建FileInputStream对象用于读取Excel文件
        ...
        for (int i = 1; i < rowCount + 1; i++) {
            ...
            String ss = row.getCell(row.getLastCellNum()-1).getStringCellValue();
